...
public class SharedPref implements ICache {
    ...
    public void putLists(String key, List<SongInfo> list) {
        Gson gson = new Gson();
        editor.putString(key, gson.toJson(list));
        editor.apply();
    }

    public List<SongInfo> getLists(String key) {
        String value = sharedPreferences.getString(key, "");
        Gson gson = new Gson();
        List<SongInfo> list = gson.fromJson(value, new TypeToken<List<SongInfo>>() {
        }.getType());
        if (list == null) {
            list = new ArrayList<>();
        }
